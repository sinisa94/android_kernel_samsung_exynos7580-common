				...
				unsigned char wdt_timer)
{
	...
	pr_battery_info("WATCHDOG_TMR set (timer=%d)\n", wdt_timer);
				...
				bool enable, bool enchg_reon)
{
	...
	pr_battery_info("ENWATCHDOG set (enable=%d, enCHG_REON=%d)\n", enable, enchg_reon);
				...
				unsigned char index, unsigned char offset)
{
	...
	if (ret < 0) {
		pr_battery_err("fail to I2C read REG:SM5705_REG_INT%d\n", 1 + index);
		return 0;
	}
				...
				unsigned char topoff_timer)
{
	...
	pr_battery_info("TOPOFF_TMR set (timer=%d)\n", topoff_timer);
				...
				bool enable)
{
	...
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL4 in AUTOSTOP[6]\n");
		return ret;
	}
				...
				unsigned short float_mV)
{
	...
	pr_battery_info("set BATREG voltage(%dmV - offset=0x%x)\n", float_mV, offset);

	ret = sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL4, offset, 0x3F);
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL4 in BATREG[5:0]\n");
		return ret;
	}
...
static unsigned short sm5705_CHG_get_BATREG(struct sm5705_charger_data *charger)
{
	...
	if (ret < 0) {
		pr_battery_err("fail to read REG:SM5705_REG_CHGCNTL4\n");
		return 0;
	}
				...
				unsigned short topoff_mA)
{
	...
	pr_battery_info("set TOP-OFF current(%dmA - offset=0x%x)\n", topoff_mA, offset);

	ret = sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL5, offset, 0xF);
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL5 in TOPOFF[3:0]\n");
		return ret;
	}
                ...
                unsigned char freq_index)
{
	...
	pr_battery_info("set BUCK&BOOST freq=0x%x\n", freq_index);

	ret = sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL5, ((freq_index & 0x3) << 4), (0x3 << 4));
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL5 in FREQSEL[5:4]\n");
		return ret;
	}
				...
				unsigned short aiclth_mV)
{
	...
	pr_battery_info("set AICL threshold (%dmV - offset=0x%x)\n", aiclth_mV, offset);

	ret = sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL7, (offset << 6), 0xC0);
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL7 in AICLTH[7:6]\n");
		return ret;
	}
...
static int sm5705_CHG_set_OVPSEL(struct sm5705_charger_data *charger, bool enable)
{
	...
	pr_battery_info("set OVPSEL=%d\n", enable);

	ret = sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL7, (enable << 2), (1 << 2));
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL7 in OVPSEL[2]\n");
		return ret;
	}
...
static int sm5705_CHG_enable_AICL(struct sm5705_charger_data *charger, bool enable)
{
	...
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL7 in AICLEN[5]\n");
		return ret;
	}
				...
				unsigned char index)
{
	if (index > SM5705_CHG_BST_IQ3LIMIT_4_0A) {
		pr_battery_err("invalid limit current index (index=0x%x)\n", index);
		return -EINVAL;
	}

	sm5705_update_reg(charger->i2c, SM5705_REG_CHGCNTL6, ((index & 0x3)), 0x3);

	pr_battery_info("BST IQ3LIMIT set (index=0x%x)\n", index);
...
static int sm5705_CHG_set_ENI2CRESET(struct sm5705_charger_data *charger, bool enable)
{
	...
	pr_battery_info("ENI2CRESET set (enable=%d)\n", enable);
	return 0;
}
#endif
				...
				unsigned char timer)
{
	...
	pr_battery_info("ENMRSTB set (timer=%d)\n", timer);
	return 0;
}
#endif
...
static int sm5705_CHG_enable_AUTOSET(struct sm5705_charger_data *charger, bool enable)
{
	...
	if (ret < 0) {
		pr_battery_err("fail to update REG:SM5705_REG_CHGCNTL7 in AUTOSET[1]\n");
		return ret;
	}
				...
				unsigned char index, unsigned short FASTCHG_mA)
{
	...
	pr_battery_info("FASTCHG src=%d, current=%dmA offset=0x%x\n", index, FASTCHG_mA, offset);
				...
				unsigned char index, unsigned short LIMIT_mA)
{
	...
	pr_battery_info("set Input LIMIT src=%d, current=%dmA offset=0x%x\n", index, LIMIT_mA, offset);
				...
				unsigned char index)
{
	...
	if (index > SM5705_CHG_SRC_WPC) {
		pr_battery_err("invalid charger source index = %d\n", index);
		return 0;
	}
	...
	pr_battery_info("get INPUT LIMIT src=%d, offset=0x%x, current=%dmA\n", index, offset, LIMIT_mA);
#endif
				...
				unsigned char index)
{
	...
	pr_battery_info("get FASTCHG src=%d, offset=0x%x, current=%dmA\n", index, offset, FASTCHG_mA);
...
static bool sm5705_charger_get_discharging_force_status(struct sm5705_charger_data *charger)
{
	...
	if (sm5705_call_fg_device_id() <= 2) {
		pr_battery_info("unsupported this function under rev02.\n");
		return false;
	}
	reg = sm5705_CHG_read_reg(charger, SM5705_REG_FACTORY);
	pr_battery_info("enable:(%s), reg[0x%02X]:0x%02x\n",
...
#ifdef SM5705_CHG_FULL_DEBUG
	pr_battery_info("src_type=%d, current=%d\n",
				...
				bool enable)
{
	...
	pr_battery_info("turn-%s Charging enable pin\n", enable ? "ON" : "OFF");
}
...
{
	if (!input_current) {
		pr_battery_info("skip process, input_current = 0\n");
		return 0;
	}
...
static void sm5705_set_current(struct sm5705_charger_data *charger)
{
	...
	pr_battery_info("(cable=%d, fast_current=%d, input_limit=%d, siop=%d)\n",
...
static void psy_chg_set_cable_online(struct sm5705_charger_data *charger, int cable_type)
{
	...
	pr_battery_info("[start] prev_cable_type(%d), cable_type(%d), op_mode(%d), op_status(0x%x)",
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_POWER_SHARING) {
		...
		charger->is_charging = false;
		pr_battery_info("OTG enable, cable(%d)\n", charger->cable_type);
		sm5705_charger_oper_push_event(SM5705_CHARGER_OP_EVENT_VBUS, 0);
		...
		if (charger->pdata->support_slow_charging) {
			pr_battery_info("request aicl work for 5V-TA\n");
	...
	pr_battery_info("[end] is_charging=%d(%d), fc = %d, il = %d, t1 = %d, t2 = %d, cable = %d,"
...
static void psy_chg_set_charge_otg_control(struct sm5705_charger_data *charger, int otg_en)
{
	...
	if (otg_en && !value.intval) {
		...
		pr_battery_info("OTG disable, cable(%d)\n", charger->cable_type);
	}
}
#endif
...
static void psy_chg_set_aicl_control(struct sm5705_charger_data *charger, int aicl_en)
{
	if (aicl_en) {
		...
		pr_battery_info("CHGIN AICL ENABLE\n");
	} else {
		sm5705_CHG_enable_AICL(charger, 0);
		pr_battery_info("CHGIN AICL DISABLE\n");
	}
}
#endif
...
{
	pr_battery_info("afc_charger_mode value = %d\n", afc_mode);
	muic_charger_init();
}
#endif
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		pr_battery_info("POWER_SUPPLY_PROP_STATUS - status=%d\n", val->intval);
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		pr_battery_info("POWER_SUPPLY_PROP_CURRENT_MAX - current=%d\n", val->intval);
		sm5705_set_input_current(charger, val->intval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		pr_battery_info("POWER_SUPPLY_PROP_CURRENT_AVG - current=%d\n", val->intval);
#if defined(CONFIG_BATTERY_SWELLING)
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		pr_battery_info("POWER_SUPPLY_PROP_CURRENT_NOW - current=%d\n", val->intval);
		sm5705_set_charge_current(charger, val->intval);
		sm5705_set_input_current(charger, val->intval);
		break;
#if defined(CONFIG_AFC_CHARGER_MODE)
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		pr_battery_info("POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN - [%d]->[%d]\n",
			charger->siop_level, val->intval);
		charger->siop_level = val->intval;
		sm5705_set_current(charger);
		break;
	case POWER_SUPPLY_PROP_USB_HC:
		pr_battery_info("POWER_SUPPLY_PROP_USB_HC - value=%d\n", val->intval);
		psy_chg_set_usb_hc(charger, val->intval);
		break;
#if defined(SM5705_SUPPORT_OTG_CONTROL)
	case POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL:
		pr_battery_info("POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL - otg_en=%d\n", val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_AICL_CONTROL:
		pr_battery_info("POWER_SUPPLY_PROP_CHARGE_AICL_CONTROL - aicl_en=%d\n", val->intval);
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		if (sm5705_call_fg_device_id() <= 2 && !charger->is_rev2_wa_done) {
			if (val->intval) {
				pr_battery_info("queue_delayed_work, op_mode_switch_work\n");
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		...
		sm5705_set_input_current(charger, charger->charging_current_max);
		pr_battery_info("%s : STORE MODE(%d)\n", __func__, charger->store_mode);
		break;
	default:
		pr_battery_err("un-known Power-supply property type (psp=%d)\n", psp);
		return -EINVAL;
	}
...
static int psy_chg_get_charging_health(struct sm5705_charger_data *charger)
{
	...
	pr_battery_info("is_charging=%d(%d), cable_type=%d, input_limit=%d, chg_curr=%d, REG_STATUS1=0x%x\n",
		charger->is_charging, sm5705_charger_get_power_source_status(charger),
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = sm5705_charger_check_oper_otg_mode_on();
		pr_battery_info("POWER_SUPPLY_PROP_ONLINE - %s\n", (val->intval) ? "ON" : "OFF");
		break;
	default:
		return -EINVAL;
	}
 ...
 static void sm5705_op_mode_switch_work(struct work_struct *work)
{
	...
	pr_battery_info("schedule work start.\n");
	...
	pr_battery_info("schedule work done.\n");
}
...
static inline unsigned int _calc_fast_chg_current_with_siop(struct sm5705_charger_data *charger)
{
	...
	if ((charger->siop_level == 0) ||
		...
		pr_battery_info("siop_level 3 detetct, we need to check this scenario\n");
	...
	pr_battery_info("cable_type=%d, cable_chg_curr=%d, siop_level=%d. "
...
static inline unsigned int _calc_input_limit_current_with_siop(struct sm5705_charger_data *charger)
{
	...
	pr_battery_info("cable_type=%d, cable_input_limit=%d, siop_level=%d, afc_detect=%d. "
...
static void sm5705_topoff_work(struct work_struct *work)
{
	...
	for (i=0; i < 3; ++i) {
		...
		pr_battery_info("%dth Check TOP-OFF state=%d\n", i, topoff);
	}
...
static void _reduce_input_limit_current(struct sm5705_charger_data *charger, int cur)
{
	...
	pr_battery_info("vbus_limit_current=%d, charger->charging_current_max=%d\n",
...
static void _check_slow_charging(struct sm5705_charger_data *charger, int input_current)
{
	...
	if (input_current <= SLOW_CHARGING_CURRENT_STANDARD &&
		...
		pr_battery_info("slow charging on : input current(%dmA), cable type(%d)\n",
...
static void sm5705_aicl_work(struct work_struct *work)
{
	...
	if (!charger->pdata->support_slow_charging || sm5705_call_fg_device_id() <= 2) {
		pr_battery_info("don't support AICL work at REV.2\n");
		return;
	}

	if (!sm5705_charger_get_charging_on_status(charger) ||
		__is_cable_type_for_hv_mains(charger->cable_type)) {
		pr_battery_info("don't need AICL work\n");
		return;
	}
	...
	if (prev_current_max > charger->charging_current_max) {
		pr_battery_info("charging_current_max(%d --> %d)\n",
			prev_current_max, charger->charging_current_max);
...
static void wc_afc_detect_work(struct work_struct *work)
{
	...
	pr_battery_info("schedule work doen.\n");
}
...
static void wpc_detect_work(struct work_struct *work)
{
	...
	if ((charger->irq_wpcin_state == 0) && (wpcin_state == 1)) {
		...
		pr_battery_info("wpc activated, set V_INT as PN\n");
	} else if ((charger->irq_wpcin_state == 1) && (wpcin_state == 0)) {
		value.intval = 0;
		psy_do_property("wireless", set, POWER_SUPPLY_PROP_ONLINE, value);

		pr_battery_info("wpc deactivated, set V_INT as PD\n");
	}

	pr_battery_info("w(%d to %d)\n", charger->irq_wpcin_state, wpcin_state);

	charger->irq_wpcin_state = wpcin_state;

	wake_unlock(&charger->wpc_wake_lock);

	pr_battery_info("%s: schedule work done.\n");
}
#endif
...
static unsigned char _get_valid_vbus_status(struct sm5705_charger_data *charger)
{
	...
	while (1) {
		...
		if (prev_vbusin == vbusin) {
			...
			pr_battery_info("VBUS status mismatch (0x%x / 0x%x), Reset stable count\n",
				vbusin, prev_vbusin);
			stable_count = 0;
		}
				...
				unsigned char vbus_status, int prev_battery_health)
{
	...
	if (vbus_status & (1 << SM5705_INT_STATUS1_VBUSPOK)) {
		...
		if ((vbus_status & (1 << SM5705_INT_STATUS1_VBUSOVP)) &&
			...
			(charger->cable_type != POWER_SUPPLY_TYPE_BATTERY)) {
			pr_battery_info("vBus is undervoltage\n");
...
static irqreturn_t sm5705_chg_vbus_in_isr(int irq, void *data)
{
	...
	pr_battery_info("battery change status [%d] -> [%d] (VBUS_REG:0x%x)\n",
...
static irqreturn_t sm5705_chg_wpcin_pok_isr(int irq, void *data)
{
	...
	if (charger->irq_wpcin_state)
		...
		delay = msecs_to_jiffies(0);
#endif
	pr_battery_info("IRQ=%d delay = %ld\n", irq, delay);
				...
				struct device_node *np, sec_charger_platform_data_t *pdata)
{
	...
	if (IS_ERR_VALUE(pdata->chg_gpio_en)) {
		pr_battery_info("can't parsing dt:battery,chg_gpio_en\n");
		return -ENOENT;
	}
	pr_battery_info("battery charge enable pin = %d\n", pdata->chg_gpio_en);

	ret = of_property_read_u32(np, "battery,siop_call_cc_current",
				&pdata->siop_call_cc_current);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_call_cc_current\n");
	}

	ret = of_property_read_u32(np, "battery,siop_call_cv_current",
				&pdata->siop_call_cv_current);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_call_cv_current\n");
	}

	ret = of_property_read_u32(np, "battery,siop_input_limit_current",
				&pdata->siop_input_limit_current);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_input_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_charging_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_hv_input_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_hv_charging_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_wireless_input_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_wireless_charging_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_hv_wireless_input_limit_current\n");
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,siop_hv_wireless_charging_limit_current\n");
...
static int _get_of_charging_current_table_max_size(struct device *dev, struct device_node *np)
{
	...
	if (unlikely(!propertys)) {
		pr_battery_info("can't parsing dt:battery,input_current_limit\n");
	} else {
		pr_battery_info("dt:battery,input_current_limit length=%d\n", len);
	}
				...
				sec_charger_platform_data_t *pdata)
{
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,chg_float_voltage\n");
	}
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,wirelss_charger_name\n");
	}
	pr_battery_info("wireless charger name=%s\n", pdata->wireless_charger_name);
	...
	if (IS_ERR_VALUE(pdata->wpc_det)) {
		pr_battery_err("can't parsing dt:battery,wpc_det\n");
		return -ENOENT;
	}
	pr_battery_info("WPC detect pin = %d\n", pdata->wpc_det);
#endif
	...
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,wpc_charging_limit_current\n");
	}

	ret = of_property_read_u32(np, "battery,sleep_mode_limit_current",
				&pdata->sleep_mode_limit_current);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,sleep_mode_limit_current\n");
	}
#endif

	ret = of_property_read_u32(np, "battery,wireless_cc_cv", &pdata->wireless_cc_cv);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,wireless_cc_cv\n");
	}
#endif

	ret = of_property_read_u32(np, "battery,full_check_type_2nd", &pdata->full_check_type_2nd);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't parsing dt:battery,full_check_type_2nd\n");
	}
	...
	pr_battery_info("dt:battery node parse done.\n");
				...
				struct sec_charger_platform_data *pdata)
{
	...
	if (np == NULL) {
		pr_battery_err("fail to find dt_node:sm5705-charger\n");
		return -ENOENT;
	} else {
	...
	if (np == NULL) {
		pr_battery_err("fail to find dt_node:battery\n");
		return -ENOENT;
	} else {
				...
				(struct platform_device *pdev, struct sm5705_charger_data *charger)
{
	...
	if (!pdata) {
		pr_battery_err("fail to memory allocate for sec_charger_platform_data\n");
		return NULL;
	}

	ret = sm5705_charger_parse_dt(charger, pdata);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("fail to parse sm5705 charger device tree (ret=%d)\n", ret);
		kfree(pdata);
		return NULL;
	}
#else
	...
	if (!pdata) {
		pr_battery_err("fail to get sm5705 charger platform data\n");
		return NULL;
	}
#endif

	pr_battery_info("Get valid platform data done. (pdata=%p)\n", pdata);
	return pdata;
}
				...
				struct sm5705_dev *sm5705, struct sm5705_charger_data *charger)
{
	...
	if (pdata == NULL) {
		pr_battery_err("can't get sm5705_platform_data\n");
		return -EINVAL;
	}

	pr_battery_info("init process start..\n");
	...
	if (charger->pdata->chg_gpio_en) {
		...
		if (ret) {
			pr_battery_err("fail to request GPIO %u\n", charger->pdata->chg_gpio_en);
			return ret;
		}
	}
	...
	if (!charger->wqueue) {
		pr_battery_err("fail to Create Workqueue\n");
		return -ENOMEM;
	}
	...
	pr_battery_info("init process done..\n");
...
{
	pr_battery_info("charger initial hardware condition process start. (float_voltage=%d)\n",
	...
	pr_battery_info("charger initial hardware condition process done.\n");
}
...
static int sm5705_charger_probe(struct platform_device *pdev)
{
	...
	pr_battery_info("Sm5705 Charger Driver Probing start\n");

	charger = kzalloc(sizeof(struct sm5705_charger_data), GFP_KERNEL);
	if (!charger) {
		pr_battery_err("fail to memory allocate for sm5705 charger handler\n");
		return -ENOMEM;
	}
	...
	if (charger->pdata == NULL) {
		pr_battery_err("fail to get charger platform data\n");
		goto err_free;
	}

	ret = _init_sm5705_charger_info(pdev, sm5705, charger);
	if (IS_ERR_VALUE(ret)) {
		pr_battery_err("can't initailize sm5705 charger");
		goto err_free;
	}
	...
	if (ret) {
		pr_battery_err("fail to register psy_chg\n");
	...
	if (ret) {
		pr_battery_err("fail to register otg_chg\n");
	...
	if (ret) {
		pr_battery_err("fail to request wpcin IRQ: %d: %d\n", charger->irq_wpcin_pok, ret);
	...
	if (ret < 0) {
		pr_battery_err("fail to request chgin IRQ: %d: %d\n", charger->irq_vbus_pok, ret);
		goto err_power_supply_register_otg;
	}
#if EN_TOPOFF_IRQ
	...
	if (ret < 0) {
		pr_battery_err("fail to request topoff IRQ: %d: %d\n", charger->irq_topoff, ret);
		goto err_power_supply_register_otg;
	}
#endif /*EN_TOPOFF_IRQ*/
#if EN_OTGFAIL_IRQ
	...
	if (ret < 0) {
		pr_battery_err("fail to request otgfail IRQ: %d: %d\n", charger->irq_otgfail, ret);
		goto err_power_supply_register_otg;
	}
#endif
	...
	if (ret){
		pr_battery_err("Failed to create_attrs\n");
		goto err_power_supply_register_otg;
	}

	pr_battery_info("SM5705 Charger Driver Loaded Done\n");
