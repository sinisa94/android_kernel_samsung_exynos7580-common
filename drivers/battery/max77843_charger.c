...
static void max77843_test_read(struct max77843_charger_data *charger)
{
	...
	for (addr = 0xB0; addr <= 0xC3; addr++) {
		...
		pr_battery_debug("MAX77843 addr : 0x%02x data : 0x%02x\n", addr, data);
	}
}
				...
				struct max77843_charger_data *charger)
{
	...
	pr_battery_info("%s : CNFG01(0x%02x)\n", __func__, chg_cnfg_01);
...
static int __devinit max77843_charger_probe(struct platform_device *pdev)
{
	...
	pr_battery_info("%s: Max77843 Charger Driver Loading\n", __func__);
	...
	pr_battery_info("%s: MAX77843 Charger Driver Loaded\n", __func__);
...
static void max77843_charger_shutdown(struct device *dev)
{
	...
	pr_battery_info("%s: MAX77843 Charger driver shutdown\n", __func__);
	if (!charger->i2c) {
		pr_battery_err("%s: no max77843 i2c client\n", __func__);
		return;
	}
	reg_data = 0x04;
	max77843_write_reg(charger->i2c,
