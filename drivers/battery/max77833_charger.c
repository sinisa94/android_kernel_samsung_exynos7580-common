...
static void max77833_test_read(struct max77833_charger_data *charger)
{
	u8 data = 0;
	u32 addr = 0;
	for (addr = 0x80; addr <= 0x9D; addr++) {
		max77833_read_reg(charger->i2c, addr, &data);
		pr_battery_debug("MAX77833 addr : 0x%02x data : 0x%02x\n", addr, data);
	}
}
...
static int max77833_get_charger_state(struct max77833_charger_data *charger)
{
	...
	pr_battery_info("%s : charger status (0x%02x)\n", __func__, reg_data);
...
static int max77833_get_charging_health(struct max77833_charger_data *charger)
{
	...
	case 0x04:
		...
		break;
	case 0x07:
		pr_battery_info("%s: battery voltage information not available\n",
			__func__);
	...
	if (state == POWER_SUPPLY_HEALTH_GOOD) {
		...
		if((charger->is_charging == 1) && (chg_dtls & 0x08)) {
			...
			pr_battery_info("%s:  CHG_CNFG_04(0x%x), CHG_CNFG_05(0x%x), CHG_CNFG_06(0x%x)\n",
				__func__, chg_cnfg_04, chg_cnfg_05, chg_cnfg_06);
			pr_battery_info("%s:  CHG_CNFG_16(0x%x), CHG_CNFG_18(0x%x)\n",
		...
		if (vbus_state == 0x02) { /*  CHGIN_OVLO */
			...
			if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS) {
				...
				if (vbus_state == 0x02) {
					state = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
					pr_battery_info("%s: wpc and over-voltage\n", __func__);
				} else
					state = POWER_SUPPLY_HEALTH_GOOD;
			}
...
static bool max77833_check_battery(struct max77833_charger_data *charger)
{
	...
	pr_battery_info("%s : CHG_INT_OK(0x%x)\n", __func__, reg_data);

	max77833_read_reg(charger->i2c,
			  MAX77833_CHG_REG_DTLS_00, &reg_data2);

	pr_battery_info("%s : CHG_DETAILS00(0x%x)\n", __func__, reg_data2);
		...
		int enable)
{
	...
	if (enable) {
		max77833_update_reg(charger->i2c, MAX77833_CHG_REG_CNFG_00,
				    ...
				    CHG_CNFG_00_OTG_CTRL | CHG_CNFG_00_BUCK_MASK);
	} else {
		max77833_update_reg(charger->i2c, MAX77833_CHG_REG_CNFG_00,
				       ...
				       int input_current)
{
	...
	pr_battery_info("[%s] REG(0x%02x) DATA(0x%02x)\n",
...
static void afc_detect_work(struct work_struct *work)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_MAINS) {
		...
		max77833_set_input_current(charger, set_input_current);
	}
}
					...
					int fast_charging_current)
{
	...
	pr_battery_info("[%s] REG(0x%02x) DATA(0x%02x), CURRENT(%d)\n",
	...
	struct max77833_charger_data *charger)
{
	...
	if ((charger->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		if ((value.intval == POWER_SUPPLY_HEALTH_UNSPEC_FAILURE) ||
		    (value.intval == POWER_SUPPLY_HEALTH_OVERHEATLIMIT)) {
			max77833_set_input_current(charger, 0);
		} else if (charger->cable_type == POWER_SUPPLY_TYPE_MAINS) {
			pr_battery_info("TA!!!\n");
			charger->afc_detect = true;
			max77833_set_input_current(charger, 1000);
	...
	pr_battery_info("charging = %d, fc = %d, il = %d, t1 = %d, t2 = %d, cable = %d\n",
		charger->is_charging,
		charger->charging_current,
		charger->charging_current_max,
...
static void max77833_chg_otype_check(struct max77833_charger_data *charger)
{
	...
	if (reg_data != 0x78) {
		...
		pr_battery_info("%s : o-type register reset.\n", __func__);
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = max77833_get_input_current(charger);
		pr_battery_debug("%s : set-current(%dmA), current now(%dmA)\n",
...
#if defined(CONFIG_BATTERY_SWELLING)
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		pr_battery_info("%s: float voltage(%d)\n", __func__, val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		...
		if (charger->is_charging) {
			...
			if ((charger->cable_type == POWER_SUPPLY_TYPE_MAINS) ||
				...
				pr_battery_info("%s: siop_level=%d, afc_detec=%d, current_max=%d, current_now=%d\n",
					__func__,
...
static int __devinit max77833_charger_probe(struct platform_device *pdev)
{
	...
	pr_battery_info("%s: Max77833 Charger Driver Loading\n", __func__);
	...
	if (charger->pdata->chg_irq) {
		...
		if (ret) {
			...
			if (ret < 0)
				pr_battery_err("%s: Failed to Enable Wakeup Source(%d)\n",
					__func__, ret);
		}
	...
	if (ret) {
		dev_battery_err(charger->dev,
			"%s : Failed to create_attrs\n", __func__);
		goto err_wc_irq;
	}
	pr_battery_info("%s: MAX77833 Charger Driver Loaded\n", __func__);
...
static void max77833_charger_shutdown(struct device *dev)
{
	...
	pr_battery_info("%s: MAX77833 Charger driver shutdown\n", __func__);
	if (!charger->i2c) {
		pr_battery_err("%s: no max77833 i2c client\n", __func__);
		return;
	}
	reg_data = 0x04;
	max77833_write_reg(charger->i2c,
