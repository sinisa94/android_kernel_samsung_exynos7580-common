...
static void max77804_dump_reg(struct max77804_charger_data *charger)
{
	...
	for (reg_addr = 0xB0; reg_addr <= 0xC5; reg_addr++) {
		...
		pr_battery_info("max77804: c: 0x%02x(0x%02x)\n", reg_addr, reg_data);
	}
}
#endif
...
static bool max77804_charger_unlock(struct max77804_charger_data *chg_data)
{
	...
	do {
		...
		if (chgprot != 0x03) {
			pr_battery_err("%s: unlock err, chgprot(0x%x), retry(%d)\n",
			...
			pr_battery_debug("%s: unlock success, chgprot(0x%x)\n",
				__func__, chgprot);
			break;
		}
...
static void check_charger_unlock_state(struct max77804_charger_data *chg_data)
{
	...
	if (need_reg_init) {
		pr_battery_err("%s: charger locked state, reg init\n", __func__);
...
static int max77804_get_battery_present(struct max77804_charger_data *charger)
{
	...
	pr_battery_debug("%s: CHG_INT_OK(0x%02x)\n", __func__, reg_data);
		...
		int enable)
{
	...
	pr_battery_debug("%s: CHG_CNFG_00(0x%02x)\n", __func__, reg_data);
...
static void max77804_check_slow_charging(struct max77804_charger_data *charger, int set_current_reg)
{
	...
	if (set_current_reg <= (SLOW_CHARGING_CURRENT_STANDARD / charger->input_curr_limit_step) &&
		...
		pr_battery_info("%s: slow charging on : set_current_reg(0x%02x), cable type(%d)\n", __func__, set_current_reg, charger->cable_type);
	}
	else
		...
		int cur)
{
	...
	if (reg_data == set_current_reg) {
		...
		while((set_current_reg > (MINIMUM_INPUT_CURRENT / charger->input_curr_limit_step)) && (set_current_reg < 255)) {
			...
			if (((vbus_state == 0x00) || (vbus_state == 0x01)) &&
				...
				pr_battery_info("%s: set_current_reg(0x%02x)\n", __func__, set_current_reg);
			...
			charger->cable_type != POWER_SUPPLY_TYPE_BATTERY)
	{
		...
		if (((vbus_state == 0x00) || (vbus_state == 0x01)) &&
			...
			max77804_write_reg(charger->max77804->i2c,
					set_reg, now_current_reg);
			pr_battery_info("%s: now_current_reg(0x%02x)\n", __func__, now_current_reg);
			chg_state = max77804_get_charger_state(charger);
			if ((chg_state != POWER_SUPPLY_STATUS_CHARGING) &&
					(chg_state != POWER_SUPPLY_STATUS_FULL))
				goto exit;
...
set_input_current:
	pr_battery_info("%s: reg_data(0x%02x), input(%d)\n",
...
static int max77804_get_input_current(struct max77804_charger_data *charger)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS) {
		...
		pr_battery_info("%s: CHG_CNFG_10(0x%02x)\n", __func__, reg_data);
		charger->input_curr_limit_step = 20;
	} else {
		max77804_read_reg(charger->max77804->i2c,
		...
		pr_battery_info("%s: CHG_CNFG_09(0x%02x)\n", __func__, reg_data);
	}
get_current = reg_data * charger->input_curr_limit_step;

	pr_battery_debug("%s: get input current: %dmA\n", __func__, get_current);
	return get_current;
}
		...
		int cur, int timeout)
{
	...
	pr_battery_info("%s: reg_data(0x%02x), topoff(%d)\n", __func__, reg_data, cur);
		...
		int cur)
{
	...
	pr_battery_info("%s: reg_data(0x%02x), charge(%d)\n",
		__func__, reg_data, cur);
}
...
static int max77804_get_charge_current(struct max77804_charger_data *charger)
{
	...
	max77804_read_reg(charger->max77804->i2c,
		MAX77804_CHG_REG_CHG_CNFG_02, &reg_data);
	pr_battery_debug("%s: CHG_CNFG_02(0x%02x)\n", __func__, reg_data);

	reg_data &= MAX77804_CHG_CC;
	get_current = reg_data * charger->charging_curr_step / 10;

	pr_battery_debug("%s: get charge current: %dmA\n", __func__, get_current);
	return get_current;
}
*/
...
static void max77804_recovery_work(struct work_struct *work)
{
	...
	if ((charger->soft_reg_recovery_cnt < RECOVERY_CNT) && (
		...
		pr_battery_info("%s: try to recovery, cnt(%d)\n", __func__,
				(charger->soft_reg_recovery_cnt + 1));

		if (charger->siop_level < 100 &&
			charger->cable_type == POWER_SUPPLY_TYPE_MAINS) {
			pr_battery_info("%s : LCD on status and revocer current\n", __func__);
		...
		pr_battery_info("%s: fail to recovery, cnt(%d)\n", __func__,
				(charger->soft_reg_recovery_cnt + 1));

		pr_battery_info("%s:  CHGIN(0x%x), CHG(0x%x), BYP(0x%x)\n",
		...
		if (charger->soft_reg_recovery_cnt < RECOVERY_CNT) {
			...
			pr_battery_info("%s: recovery cnt(%d) is over\n",
...
static void reduce_input_current(struct max77804_charger_data *charger, int cur)
{
	...
	if (!max77804_read_reg(charger->max77804->i2c,
		...
		max77804_write_reg(charger->max77804->i2c,
				set_reg, set_value);
		pr_battery_info("%s: set current: reg:(0x%x), val:(0x%x)\n",
...
static int max77804_get_vbus_state(struct max77804_charger_data *charger)
{
	...
	switch (reg_data) {
	case 0x00:
		pr_battery_info("%s: VBUS is invalid. CHGIN < CHGIN_UVLO\n",
			__func__);
		break;
	case 0x01:
		pr_battery_info("%s: VBUS is invalid. CHGIN < MBAT+CHGIN2SYS" \
			"and CHGIN > CHGIN_UVLO\n", __func__);
		break;
	case 0x02:
		pr_battery_info("%s: VBUS is invalid. CHGIN > CHGIN_OVLO",
			__func__);
		break;
	case 0x03:
		pr_battery_info("%s: VBUS is valid. CHGIN < CHGIN_OVLO", __func__);
		break;
	default:
		break;
	}
...
static int max77804_get_charger_state(struct max77804_charger_data *charger)
{
	...
	pr_battery_info("%s: CHG_DTLS : 0x%2x\n", __func__, reg_data);
...
static int max77804_get_health_state(struct max77804_charger_data *charger)
{
	...
	pr_battery_info("%s: reg_data(0x%x)\n", __func__, reg_data);
	switch (reg_data) {
	case 0x00:
		pr_battery_info("%s: No battery and the charger is suspended\n",
			__func__);
		state = POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
		break;
	case 0x01:
		pr_battery_info("%s: battery is okay "
			"but its voltage is low(~VPQLB)\n", __func__);
		state = POWER_SUPPLY_HEALTH_GOOD;
		break;
	case 0x02:
		pr_battery_info("%s: battery dead\n", __func__);
	...
	case 0x04:
		...
		break;
	case 0x05:
		pr_battery_info("%s: battery ovp\n", __func__);
		state = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
		break;
	default:
		pr_battery_info("%s: battery unknown : 0x%d\n", __func__, reg_data);
	...
	if (state == POWER_SUPPLY_HEALTH_GOOD) {
		...
		max77804_read_reg(charger->max77804->i2c,
			...
			pr_battery_info("%s: CHG_DTLS_00(0x%x), CHG_DTLS_01(0x%x), CHG_CNFG_00(0x%x)\n",
					__func__, chg_dtls_00, chg_dtls, chg_cnfg_00);
			pr_battery_info("%s:  CHG_CNFG_01(0x%x), CHG_CNFG_02(0x%x), CHG_CNFG_04(0x%x)\n",
					__func__, chg_cnfg_01, chg_cnfg_02, chg_cnfg_04);
			pr_battery_info("%s:  CHG_CNFG_09(0x%x), CHG_CNFG_12(0x%x)\n",
					__func__, chg_cnfg_09, chg_cnfg_12);
		}

		pr_battery_info("%s: vbus_state : 0x%d, chg_dtls : 0x%d\n", __func__, vbus_state, chg_dtls);
		/*  OVP is higher priority */
		if (vbus_state == 0x02) { /*  CHGIN_OVLO */
			pr_battery_info("%s: vbus ovp\n", __func__);
			state = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
			if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS) {
				retry_cnt = 0;
				do {
				...
				(charger->cable_type != POWER_SUPPLY_TYPE_WIRELESS)) {
			pr_battery_info("%s: vbus is under\n", __func__);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		else if (charger->aicl_on)
		{
			...
			pr_battery_info("%s: slow-charging mode\n", __func__);
		}
		else
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (val->intval == POWER_SUPPLY_TYPE_POWER_SHARING) {
			...
			if (value.intval) {
				...
				pr_battery_info("%s: ps enable\n", __func__);
			} else {
				max77804_update_reg(charger->max77804->i2c, MAX77804_CHG_REG_CHG_CNFG_00,
					0, chg_cnfg_00);
				pr_battery_info("%s: ps disable\n", __func__);
			}
			break;
		}
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
	{
		...
		pr_battery_info("%s: ctrl3 : (0x%02x)\n", __func__, ctrl3);
		pr_battery_info("%s: set CNFG_12: 0x%x\n", __func__, cnfg12);
		break;
	}
#endif
	default:
...
static void max77804_charger_initialize(struct max77804_charger_data *charger)
{
	...
	max77804_read_reg(charger->max77804->i2c,
			MAX77804_CHG_REG_CHG_CNFG_04, &reg_data);
	pr_battery_info("%s: battery cv voltage 0x%x\n", __func__, reg_data);
}
...
static void sec_chg_isr_work(struct work_struct *work)
{
	...
	if (charger->pdata->full_check_type ==
		...
		case POWER_SUPPLY_STATUS_DISCHARGING:
			pr_battery_err("%s: Interrupted but Discharging\n", __func__);
			break;

		case POWER_SUPPLY_STATUS_NOT_CHARGING:
			pr_battery_err("%s: Interrupted but NOT Charging\n", __func__);
			break;

		case POWER_SUPPLY_STATUS_FULL:
			pr_battery_info("%s: Interrupted by Full\n", __func__);
			psy_do_property("battery", set,
				POWER_SUPPLY_PROP_STATUS, val);
			break;

		case POWER_SUPPLY_STATUS_CHARGING:
			pr_battery_err("%s: Interrupted but Charging\n", __func__);
			break;

		case POWER_SUPPLY_STATUS_UNKNOWN:
		default:
			pr_battery_err("%s: Invalid Charger Status\n", __func__);
			break;
		}
	}
	...
	if (charger->pdata->ovp_uvlo_check_type ==
		...
		case POWER_SUPPLY_HEALTH_COLD:
			pr_battery_err("%s: Interrupted but Hot/Cold\n", __func__);
			break;

		case POWER_SUPPLY_HEALTH_DEAD:
			pr_battery_err("%s: Interrupted but Dead\n", __func__);
			break;

		case POWER_SUPPLY_HEALTH_OVERVOLTAGE:
		case POWER_SUPPLY_HEALTH_UNDERVOLTAGE:
			pr_battery_info("%s: Interrupted by OVP/UVLO\n", __func__);
			psy_do_property("battery", set,
				POWER_SUPPLY_PROP_HEALTH, val);
			break;

		case POWER_SUPPLY_HEALTH_UNSPEC_FAILURE:
			pr_battery_err("%s: Interrupted but Unspec\n", __func__);
			break;

		case POWER_SUPPLY_HEALTH_GOOD:
			pr_battery_err("%s: Interrupted but Good\n", __func__);
			break;

		case POWER_SUPPLY_HEALTH_UNKNOWN:
		default:
			pr_battery_err("%s: Invalid Charger Health\n", __func__);
			break;
		}
	}
}
...
static irqreturn_t sec_chg_irq_thread(int irq, void *irq_data)
{
	...
	pr_battery_info("%s: Charger interrupt occured\n", __func__);
...
static void wpc_detect_work(struct work_struct *work)
{
	...
	if ((chg_data->wc_w_state == 0) && (wc_w_state == 1)) {
		...
		pr_battery_info("%s: wpc activated, set V_INT as PN\n",
				__func__);
		...
		if ((reg_data != 0x08)
			...
			psy_do_property("wireless", set,
					POWER_SUPPLY_PROP_ONLINE, value);
			pr_battery_info("%s: wpc deactivated, set V_INT as PD\n",
					__func__);
		}
	}
	pr_battery_info("%s: w(%d to %d)\n", __func__,
...
static irqreturn_t max77804_batp_irq(int irq, void *data)
{
	...
	pr_battery_info("%s: battery present interrupt occured\n", __func__);
	max77804_read_reg(chg_data->max77804->i2c,
			MAX77804_CHG_REG_CHG_INT_OK, &batp_ok);

	max77804_read_reg(chg_data->max77804->i2c,
			MAX77804_CHG_REG_CHG_DTLS_00, &bat_dtls);
	pr_battery_info("%s : CHG_INT_OK : 0x%x, CHG_DTLS_00: 0x%x\n",
...
static irqreturn_t max77804_bypass_irq(int irq, void *data)
{
	...
	byp_dtls = ((dtls_02 & MAX77804_BYP_DTLS) >>
				MAX77804_BYP_DTLS_SHIFT);
	pr_battery_info("%s: BYP_DTLS(0x%02x)\n", __func__, byp_dtls);
	vbus_state = max77804_get_vbus_state(chg_data);

	if (byp_dtls & 0x1) {
		pr_battery_info("%s: bypass overcurrent limit\n", __func__);
#ifdef CONFIG_USB_HOST_NOTIFY
			...
			if (charger->is_charging) {
				if ((chgin_dtls == 0x02) && \
					...
					psy_do_property("battery", set,
						...
						(charger->cable_type != POWER_SUPPLY_TYPE_WIRELESS)) {
					pr_battery_info("%s, vbus_state : 0x%d, chg_state : 0x%d\n", __func__, chgin_dtls, chg_dtls);
					pr_battery_info("%s: vBus is undervoltage\n", __func__);
					...
					psy_do_property("battery", set,
						...
						(chgin_dtls != 0x02)) {
					pr_battery_info("%s: vbus_state : 0x%d, chg_state : 0x%d\n", __func__, chgin_dtls, chg_dtls);
					pr_battery_info("%s: overvoltage->normal\n", __func__);
					...
					pr_battery_info("%s: vbus_state : 0x%d, chg_state : 0x%d\n", __func__, chgin_dtls, chg_dtls);
					pr_battery_info("%s: undervoltage->normal\n", __func__);
					value.intval = POWER_SUPPLY_HEALTH_GOOD;
					psy_do_property("battery", set,
							POWER_SUPPLY_PROP_HEALTH, value);
					max77804_set_input_current(charger,
...
static void max77804_chgin_init_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		pr_battery_err("%s: fail to request chgin IRQ: %d: %d\n",
...
static __devinit int max77804_charger_probe(struct platform_device *pdev)
{
	...
	pr_battery_info("%s: MAX77804 Charger driver probe\n", __func__);
...
/*
	if (charger->pdata->chg_gpio_init) {
		if (!charger->pdata->chg_gpio_init()) {
			pr_battery_err("%s: Failed to Initialize GPIO\n", __func__);
			goto err_free;
		}
	}
*/
	...
	if (max77804_read_reg(charger->max77804->i2c, MAX77804_PMIC_REG_PMIC_ID1, &reg_data) < 0) {
		pr_battery_err("device not found on this channel (this is not an error)\n");
		ret = -ENODEV;
		goto err_free;
	} else {
		charger->pmic_ver = (reg_data & 0xf);
		pr_battery_info("%s: device found: ver.0x%x\n", __func__,
				charger->pmic_ver);
	}
	...
	if (!charger->wqueue) {
		pr_battery_err("%s: Fail to Create Workqueue\n", __func__);
		goto err_free;
	}
	...
	if (ret < 0)
		pr_battery_err("%s: fail to request bypass IRQ: %d: %d\n",
	...
	if (ret < 0)
		pr_battery_err("%s: fail to request batp irq : %d, %d\n",
...
static void max77804_charger_shutdown(struct device *dev)
{
	...
	pr_battery_info("%s: MAX77804 Charger driver shutdown\n", __func__);
	if (!charger->max77804->i2c) {
		pr_battery_err("%s: no max77804 i2c client\n", __func__);
		return;
	}
	reg_data = 0x04;
	max77804_write_reg(charger->max77804->i2c,
