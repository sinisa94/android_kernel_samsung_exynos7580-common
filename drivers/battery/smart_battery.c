...
static bool sec_bat_battery_cable_check(struct sec_battery_info *battery)
{
	...
	dev_battery_dbg(battery->dev, "%s: Battery Connected\n", __func__);
...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	pr_battery_info("%s: status(%d), swell_mode(%d:%d), cv(%d), temp(%d)\n",
	...
	if (!battery->swelling_mode) {
		if (((temperature >= battery->pdata->swelling_high_temp_block) ||
			...
			if (temperature >= battery->pdata->swelling_high_temp_block &&
				...
				!battery->event) {
				pr_battery_info("%s: skip check swelling in high temperature event mode(%d)\n",
	...
	if (battery->polling_in_sleep) {
		...
		if ((battery->status == POWER_SUPPLY_STATUS_DISCHARGING) &&
			...
			if ((unsigned long)(c_ts.tv_sec - old_ts.tv_sec) < 10 * 60) {
				pr_battery_info("Skip monitor_work(%ld)\n",
...
static irqreturn_t sec_cable_irq_thread(int irq, void *irq_data)
{
	...
	dev_battery_info(battery->dev, "%s:(cable_irq occured_start\n", __func__);

	if (battery->pdata->cable_check_type &
		SEC_BATTERY_CABLE_CHECK_INT) {
		if(gpio_get_value(battery->pdata->cable_irq_gpio)) {
			pr_battery_info("PIN HIGH\n");
		...
		struct sec_battery_info *battery)
{
	...
	if (ret)
		pr_battery_info("%s: swelling drop float voltage is Empty\n", __func__);
	...
	if (ret)
		pr_battery_info("%s: swelling_block_time is Empty\n", __func__);
